...
class NetworkManager {
	...
	func updatePaginator(category:String) {
		...
		if let image = self.imageCache.objectForKey(imageURL!) as? UIImage {
			...
			request?.responseImageData({ (request, _, data, error) -> Void in
				if error == nil && data != nil {
					dispatch_async(dispatch_get_global_queue(Int(QOS_CLASS_BACKGROUND.rawValue), 0)) { // 1
	...
	func getImageFromSystemCacheForURL(urlString:String)-> UIImage? {
		let image:UIImage? = self.imageCache.objectForKey(urlString) as? UIImage
		return image
	}
}
